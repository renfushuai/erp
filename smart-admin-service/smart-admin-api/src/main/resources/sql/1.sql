...
-- 正在导出表  smart-admin-dev.erp_company 的数据：~2 rows (大约)
DELETE FROM `erp_company`;
...
CREATE TABLE IF NOT EXISTS `erp_hospital` (
    ...
    `create_time` datetime DEFAULT NULL COMMENT '创建时间',
    PRIMARY KEY (`id`)
    ) ENGINE=InnoDB AUTO_INCREMENT=109 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='医院';

-- 正在导出表  smart-admin-dev.erp_hospital 的数据：~0 rows (大约)
DELETE FROM `erp_hospital`;
...
-- 正在导出表  smart-admin-dev.erp_sales_order 的数据：~300 rows (大约)
...
CREATE TABLE IF NOT EXISTS `erp_sales_order_info` (
    ...
    `standard_price` decimal(20,6) DEFAULT NULL COMMENT '出货单价',
    `update_time` datetime DEFAULT NULL,
    `create_time` datetime DEFAULT NULL,
    PRIMARY KEY (`id`),
...
DELETE FROM `erp_sales_order_info`;
